...
public class BetterFunnelCommand extends GBCommand {
    ...
    public static final double pusherDelay = 500; // in milliseconds
    ...
    @Override
    public void execute() {
        SmartDashboard.putBoolean("Shoot", Math.abs(Shooter.getInstance().getShooterSpeed() - this.targetSpeed) < deltaLimit);
        SmartDashboard.putNumber("Delta Speed", Math.abs(Shooter.getInstance().getShooterSpeed() - this.targetSpeed));
        if (Math.abs(Shooter.getInstance().getShooterSpeed() - this.targetSpeed) < deltaLimit) {
            if (System.currentTimeMillis() - tStart > pusherDelay) {
                push.execute();
            }
            insert.execute();
        } else {
            ...
            this.push = new PushByDifferentConstants(0.8, 0.3);
            this.insert = new InsertByConstant(0.8);
        }
    }
